    ...
    if (new.pagesize > curitems) {
        ...
        for (i = 0; i < new.pagesize - curitems; ++i) {
            ...
            if (!edje_object_file_set
                (frame, data->frame_theme_file, data->frame_theme_group))
                exit(17);
            ...
            if (!edje_object_file_set
                (item, data->item_theme_file, data->item_theme_group))
                exit(18);
...
choicebox_new(Evas * evas, choicebox_info_t * info, void *param)
{
    ...
    if (info->background)
        ...
        if (!edje_object_file_set
            (data->background, DATADIR "/choicebox/background.edj",
             "background")) {
            evas_object_del(data->background);
    ...
    if (!edje_object_file_set
        (tmpitem, data->frame_theme_file, data->frame_theme_group)) {
